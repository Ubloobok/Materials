...
| Problem Name | Approach | Complexity | Real-World Examples | 
|--------------|----------|------------|---------------------|
| [121. Best Time to Buy and Sell Stock](https://leetcode.com/problems/best-time-to-buy-and-sell-stock/) | Iterate through once. Keep the min price and max profit from potential sales on subsequent days. | O(n) Time, O(1) Space | Used in financial algorithms to maximize profit from trading stocks based on historical data. |
...
| [424. Longest Repeating Character Replacement](https://leetcode.com/problems/longest-repeating-character-replacement/) | Iterate through with start and end. Keep counts of characters and max count (assumption: the longest sequence contains max frequent character), check `if (end - start + 1 - maxCount > k)`, then reduce window and count. | O(n) Time, O(m) Space | Modifying strings in text processing to achieve uniformity for aesthetic or formatting purposes. |
| [567. Permutation in String](https://leetcode.com/problems/permutation-in-string/) | Iterate through with start and end. Keep the window length equal to `s1` length. Count characters in `s1`, which should be at least equal to matches counts, and they sequence should be equal to counts in `s2` window. | O(n) Time, O(m) Space | Searching for anagrams or certain patterns within biological sequences in computational biology. |
| [76. Minimum Window Substring](https://leetcode.com/problems/minimum-window-substring/) | Iterate through with start and end. Keep and shrink dynamically the window to find the smallest substring. Matches count should be equal to target length. Return substring by start and end. | O(n) Time, O(m) Space | Filtering data to find the minimum context window containing required tokens in natural language processing. |
| [239. Sliding Window Maximum](https://leetcode.com/problems/sliding-window-maximum/) | Naive approach is quadratic, nested loop. Better approach with doubled queue (use linked list). Iterate through with start and end. At each window step, remove first (dequeue on head), remove last while it is smaller than right value (dequeue on tail). So, the front of the deque always gives the maximum element for the current window. | O(n) Time, O(k) Space | Real-time queue management where maximum or priority needs to be determined quickly within a moving time frame. |
